...
contract GreenEnergyExchange {
    ...
    function buyCredits(uint256 _listingId, uint256 _amount) public payable {
        ...
        emit CreditsSold(_listingId, msg.sender, _amount);
    }
    
    /**
     * @dev Donate credits to offset someone else's carbon footprint
     */
    function donateCredits(address _recipient, uint256 _amount) public {
        require(creditBalance[msg.sender] >= _amount, "Insufficient credits");
        require(_amount > 0, "Amount must be positive");
        
        creditBalance[msg.sender] -= _amount;
        creditBalance[_recipient] += _amount;
        
        producers[msg.sender].totalDonated += _amount;
        totalCreditsDonated += _amount;
        
        emit CreditsDonated(msg.sender, _recipient, _amount);
    }
    
    /**
     * @dev Cancel a listing and return credits
     */
    function cancelListing(uint256 _listingId) public {
        ...
        creditBalance[msg.sender] += listing.amount;
        listing.active = false;
        listing.amount = 0;
    }
    ...
    function getMyCredits() public view returns (uint256) {
        return creditBalance[msg.sender];
    }
    ...
    /**
     * @dev Get producer info
     */
    function getProducerInfo(address _producer) public view returns (
        string memory name,
        uint256 totalProduced,
        uint256 totalDonated,
        bool verified
    ) {
        Producer memory p = producers[_producer];
        return (p.name, p.totalProduced, p.totalDonated, p.verified);
    }
}